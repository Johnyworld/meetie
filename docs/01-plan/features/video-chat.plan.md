...
| v4 | 2026-03-16 | **참여자 리스트** (채팅 패널 내 현재 참여자 ID 표시) 추가 |
...
- **참여자 리스트** (채팅 패널 상단에 현재 접속 중인 참여자 ID 목록 표시) ← **NEW v4**
...
| US-09 | 참여자 | 현재 방에 누가 있는지 이름(ID) 목록을 보고 싶다 | 함께 있는 사람 파악 |
...
- [ ] 채팅 패널 상단에 현재 참여자 수와 ID 목록이 표시된다
- [ ] 참여자 입장/퇴장 시 목록이 실시간으로 업데이트된다
